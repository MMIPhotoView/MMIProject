...
function ActivityDatabase(){
     ...
     * @param callback
     */
    this.addUserinfo = function (userinfo, callback) {
        if (!userinfo) {
            callback && callback('params error');
            return ;
        }
        db.run("insert into t_userinfo values(?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [null,userinfo.aid,userinfo.age,userinfo.sex,userinfo.name,userinfo.desc,userinfo.phone,userinfo.birthday,userinfo.hobby],
            function(err) {
                if (err) {
                    ...
                    callback && callback();
                    console.log('用户aid:'+userinfo.aid+'注册成功');
                }
            });
    }
